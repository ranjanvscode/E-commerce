...
public class EmailService {
    ...
    public boolean sendEmail(String toEmail, String subject, String htmlContent) {
        ...
        if (brevoApiKey == null || brevoApiKey.isBlank()) {
            System.err.println("Brevo API key is not set (null/blank). Check configuration.");
        } else {
            System.out.println("Brevo API key length: " + brevoApiKey.length() + " (masked): " + (brevoApiKey.length() > 8 ? brevoApiKey.substring(0,4) + "..." + brevoApiKey.substring(brevoApiKey.length()-4) : "masked"));
        }

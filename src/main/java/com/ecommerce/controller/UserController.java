...
public class UserController {
    ...
    public UserController(UserServiceImpl userService, ImageService imageService, EmailService emailService) {
    ...
    public ResponseEntity<Map<String, String>> doRegister(@Valid @RequestBody UserRequest userForm, BindingResult errorResult) {
        ...
        if (data == null) {
            ...
            emailService.sendRegistrationEmail(userForm.getEmail(), userForm.getName());
    ...
    public ResponseEntity<String> putMethodName(@PathVariable String id, @RequestBody ProfileRequest profileRequest) {
        ...
        user.setEnabled(profileRequest.isEnabled());
        String role = profileRequest.getUserRole();
        user.setRoleList(role.equalsIgnoreCase("user")  
            ? new ArrayList<>(List.of(AppConstant.ROLE_USER))
            : new ArrayList<>(List.of(AppConstant.ROLE_USER, AppConstant.ROLE_ADMIN)));
